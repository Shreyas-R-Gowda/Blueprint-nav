...
import numpy as np
import cv2
from grid_generator import generate_grid, inflate_obstacles, visualise_grid
...
# ── PATH TO COMMANDS ───────────────────────────────────────────────────────
def path_to_commands(path, cell_size_cm=10, initial_heading='N'):
    """
    Convert grid path to robot motion commands.
    Works on raw A* path (step by step) for reliable direction detection.
    Returns list like ['R90', 'F190', 'L90', 'F200']
    """
    if len(path) < 2:
        return []
    ...
    TURN = {
        'N': {'N': None, 'E': 'R90', 'S': 'R180', 'W': 'L90'},
        'E': {'E': None, 'S': 'R90', 'W': 'R180', 'N': 'L90'},
        'S': {'S': None, 'W': 'R90', 'N': 'R180', 'E': 'L90'},
        'W': {'W': None, 'N': 'R90', 'E': 'R180', 'S': 'L90'},
    }
    ...
    for i in range(1, len(path)):
        ...
        move_dir = DIR_VECTOR.get((dr, dc))

        if move_dir is None:
            continue  # skip diagonal (shouldn't happen with 4-dir A*)

        if move_dir != current_heading:
            # Flush accumulated forward movement first
            if forward_count > 0:
                commands.append(f"F{forward_count * cell_size_cm}")
                forward_count = 0
            # Add turn
            turn = TURN[current_heading][move_dir]
            if turn:
                commands.append(turn)
            current_heading = move_dir

        forward_count += 1

    # Flush any remaining forward movement
    if forward_count > 0:
        commands.append(f"F{forward_count * cell_size_cm}")
